...
if __name__ == "__main__":
    ...
    if model_type == 'vae':
        runner = VAEInferencer(output_dir=args.outputdir, save_plot=args.plot, checkpoint_path=args.checkpoint, 
                                data_path=args.data_path, conversion_dict_path=args.conversion_dict, 
                                sample_frac=args.sample_frac, hparams=hparams, 
                                batch_size=args.batch_size, data_dir=args.data_dir)
    else:
        runner = PairVAEInferencer(mode=args.mode, output_dir=args.outputdir, save_plot=args.plot, checkpoint_path=args.checkpoint, 
                                    data_path=args.data_path,  conversion_dict_path=args.conversion_dict, 
                                    sample_frac=args.sample_frac, hparams=hparams, 
                                    batch_size=args.batch_size, data_dir=args.data_dir)
    runner.infer()
