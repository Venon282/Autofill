...
PATH_DATABASE = os.path.join(BASE_DIR, "Mat_Cext_Ag_cylinder")
...
def fit_cyl_Ag(T,Longueur,wl):
    ...
    assert np.all((T >= 0) & (T <= 1)), "T doit être entre 0 et 1"
    assert Longueur > 0, "Longueur doit être positive"
    assert np.all((wl >= 300) & (wl <= 1100)), "wl doit être entre 300 et 1100 nm"
