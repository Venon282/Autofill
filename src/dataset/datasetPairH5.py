...
class PairHDF5Dataset(Dataset):
    ...
    def __init__(self, hdf5_file, conversion_dict: Union[dict, str, Path], metadata_filters=None,
        ...
        if self.use_data_q:
            ...
            self.data_q_saxs = None
            self.data_q_les = None

        assert len(self.data_y_saxs) > 0, (
        ...
        if conversion_dict is not None:
            ...
            self.filtered_indices = list(range(len(self.data_y_saxs)))
    ...
    def __getitem__(self, idx):
        ...
        if self.use_data_q and self.data_q_saxs is not None and self.data_q_les is not None:
            data_q_saxs = self.transformer_q_saxs.transform(self.data_q_saxs)
            data_q_saxs = torch.as_tensor(data_q_saxs, dtype=torch.float32).unsqueeze(0)
            data_q_les = self.transformer_q_les.transform(self.data_q_les)
            data_q_les = torch.as_tensor(data_q_les, dtype=torch.float32).unsqueeze(0)
        else:
            data_q_saxs = None
            data_q_les = None

        data_y_saxs = self.transformer_y_saxs.transform(data_y_saxs)
        data_y_saxs = torch.as_tensor(data_y_saxs, dtype=torch.float32).unsqueeze(0)
        data_y_les = self.transformer_y_les.transform(data_y_les)
        data_y_les = torch.as_tensor(data_y_les, dtype=torch.float32).unsqueeze(0)
